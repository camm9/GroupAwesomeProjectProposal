		...
		if(token_s == null) {
			...
			if(sizePreferedMID > 0 ) {
				for(int i=0; i < sizePreferedMID ; i++ ) {
					...
					if(!dateIdUser.contains(userDatasetService.findDataSetbyToken(Long.parseLong(token_s)).get(i).getDos())) {
...
//		System.out.print(searchByMatchID);
	...
	@PostMapping("/member")
	public String renderAPIDateMember(@RequestParam("date_p") String date_p, @RequestParam("token") String token, Model model) {
	...
	public String renderSaveDatasetMember(@RequestParam("rnd") String rnd, @RequestParam("matchid") String matchid, @RequestParam("tok") String token, @RequestParam("dos") String dos, @RequestParam("dose") String date_p) {
